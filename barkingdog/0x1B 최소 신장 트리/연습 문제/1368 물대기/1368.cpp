...
이 풀이에서는 각 노드마다 우물 역할을 하는 노드를 하나씩 추가로 두고, 논 노드와 우물 노드를 가중치 0인 간선으로
연결했다. 이후 각 논 노드와 우물 노드는 다른 모든 노드와 간선을 연결하는데, 논 노드는 다른 논과 잇는 비용이고, 우물
노드는 다른 논에 상관없이 우물을 파는 비용이다. 이렇게 그래프를 변형해 MST를 구하면 그게 답이 된다. (이미 이어져있는 논,
우물 노드에해대해 다른 노드와 연결하거나, 우물을 파거나 둘 중 하나면 연결되기 때문이다.)
...
# 직접 그려보면서 어떻게 그래프를 변형할 수 있을지 고민해보자. '어떻게 하면 우물을 파는 상황을 표현할 수 있을까?'
# 그래프를 변형하는 유형은 정말 자주 나오는 유형이다. 어떤 문제에 그래프를 적용할 수 있을 것 같은데 걸리는 부분이
있다면, 그래프를 변형하여 그 부분을 어떻게 해결할 수 있을지 고민해보자.
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        to_visit.push_back({cost[i][i + n], i, i + n});
    }
