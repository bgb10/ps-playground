...
const int M = 1000003; // 해시 함수의 주기(크기) 또는 해시 테이블의 크기
const int a = 1000;

const int MX = 500005; // 최대 삽입 횟수
int head[M]; // 해시에 대한 연결리스트의 주소 저장
int pre[MX];
int nxt[MX];
string key[MX];
int val[MX];
...
void erase(string k){
  ...
  int h = my_hash(k);
  if(head[h] == idx) head[h] = nxt[idx]; // 지우려는 값이 head 노드였다면 지우려는 값의 다음 노드를 head 노드로 한다.
}
